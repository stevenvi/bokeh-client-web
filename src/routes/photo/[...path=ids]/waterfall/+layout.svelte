<script lang="ts">
	...
	$effect(() => {
		...
		loadCollectionChain(ids).then((collections) => {
			...
			// Waterfall is a visible breadcrumb entry so back from waterfall slideshow
			// returns here, and back from here returns to the album view.
			applyBreadcrumbs([
				...collectionEntries,
				{ id: leaf.id, name: 'Waterfall', path: '/photo/' + path + '/waterfall' }
			]);
			leafCollection = leaf;
			loadState = 'loaded';
		});
	...
	$effect(() => {
		...
		toolbarStore.set({
			mode: 'waterfall',
			onModeChange: (m) => { if (m === 'album') goto(bp); }
		});
