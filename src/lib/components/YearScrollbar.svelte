<script lang="ts">
	...
	interface Props {
		...
		onJump?: (date: string, fractionWithin: number) => void;
	}

	let { metadata, onJump }: Props = $props();
	...
	function recomputeIndicator() {
		if (dragging) return;
		const l = layout();
		if (!l || !containerRect) return;
		const centerY = containerRect.top + containerRect.height / 2;
		const elements = document.querySelectorAll<HTMLElement>('[data-date]');
		if (elements.length === 0) return;

		// Find the loaded month whose vertical extent contains the container center.
	...
	$effect(() => {
		// Re-run whenever the layout (metadata) changes so the initial indicator
		// reflects loaded data once stats arrive.
		layout();
		recomputeIndicator();
	});

	// Setup-only effect: attach listeners and seed the initial container rect.
	// We deliberately do NOT call recomputeIndicator() here, because this effect
	// writes `containerRect` and recomputeIndicator reads it — that combination
	// would register a self-update and throw "Maximum update depth exceeded".
	// The neighboring `$effect(() => { layout(); recomputeIndicator(); })` runs
	// when containerRect/metadata change, so it covers the initial recompute.
	$effect(() => {
	...
	function scrollToProportionIfLoaded(proportion: number) {
		...
		if (!target || !scrollContainer || !containerRect) return;
		...
		scrollContainer.scrollTo(0, Math.max(0, scrollContainer.scrollTop + offset));
	}
	...
	function onPointerUp() {
		...
		if (dragProportion != null) {
			...
			if (target) {
				const el = document.querySelector<HTMLElement>(`[data-date="${target.date}"]`);
				if (!el && onJump) {
					onJump(target.date, target.fractionWithin);
				}
			}
		}
		dragProportion = null;
		// Sync indicator to where scrolling actually landed.
		recomputeIndicator();
	}
...
{#if layout() && containerRect}
	...
	<div
		...
		style="top: {containerRect.top}px; height: {containerRect.height}px; touch-action: none;"
		onpointerdown={onPointerDown}
		onpointermove={onPointerMove}
		onpointerup={onPointerUp}
		onpointercancel={onPointerUp}
		role="scrollbar"
		aria-controls="app-scroll"
	...
	>
			...
			>
				...
				{#if l.singleYear}
					<div class="absolute left-0 right-0 top-0 flex items-center justify-center">
						<span class="text-text-secondary px-0.5 text-[11px] leading-none lg:text-xs">
							{seg.label}
						</span>
					</div>
				{/if}
		...
		{#each l.yearLabelPositions as lbl (lbl.year)}
			...
			>
				<span class="text-text-secondary px-0.5 text-xs font-semibold leading-none lg:text-sm">
					{lbl.year}
				</span>
			</div>
		{/each}
